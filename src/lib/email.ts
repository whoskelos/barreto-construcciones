...
export async function sendAutoResponseEmail(data: ContactFormData): Promise<EmailResult> {
  const { nombre, apellido, email, asunto } = data;

  if (!email) {
    return { success: false, error: 'No email provided for auto-response' };
  }

  // HTML Template for Auto-response
  const emailHtml = `
    <!DOCTYPE html>
    <html lang="es">
      <head>
      ...
      <body style="font-family: 'Roboto', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #18181b; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f5f5f4;">
        <div style="background-color: #ed6309; padding: 30px; border-radius: 8px 8px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px; font-weight: 700;">¡Gracias por contactarnos!</h1>
          <p style="color: #fff7ed; margin: 5px 0 0 0; font-size: 14px;">Barreto Construcciones</p>
        </div>
        
        <div style="background: white; padding: 30px; border-radius: 0 0 8px 8px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
          <div style="margin-bottom: 25px;">
            <p style="font-size: 16px; color: #3f3f46;">Hola <strong>${nombre} ${apellido}</strong>,</p>
            <p style="color: #3f3f46;">Hemos recibido tu mensaje correctamente. Nuestro equipo revisará tu consulta y nos pondremos en contacto contigo lo antes posible.</p>
          </div>
          
          <div style="background-color: #fafaf9; padding: 20px; border-radius: 6px; border: 1px solid #e7e5e4; margin-bottom: 25px;">
            <h3 style="color: #ea580c; margin-top: 0; font-size: 16px; margin-bottom: 10px;">Resumen de tu mensaje:</h3>
            <p style="margin: 0 0 5px 0; font-size: 14px;"><strong>Asunto:</strong> ${asunto}</p>
          </div>
          
          <div style="margin-top: 25px; padding-top: 20px; border-top: 1px solid #e7e5e4;">
            <p style="margin: 0; color: #3f3f46;">Atentamente,</p>
            <p style="margin: 5px 0 0 0; font-weight: bold; color: #ea580c;">El equipo de Barreto Construcciones</p>
          </div>
        </div>
  ...
  // Plain Text Template for Auto-response
  const emailText = `
Hola ${nombre} ${apellido},

Hemos recibido tu mensaje correctamente. Nuestro equipo revisará tu consulta y nos pondremos en contacto contigo lo antes posible.

Resumen de tu mensaje:
Asunto: ${asunto}

Atentamente,
El equipo de Barreto Construcciones
...
Este es un mensaje automático, por favor no respondas a este correo.
  ...
  try {
    const { data: emailData, error } = await resend.emails.send({
      ...
      text: emailText,
      tags: [
        { name: 'tipo', value: 'auto-respuesta' },
        { name: 'origen', value: 'web' },
      ],
    });
    ...
    console.error('Unexpected auto-response error:', error);
    return { success: false, error };
  }
}
